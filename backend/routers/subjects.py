...
def delete_subject(subject_id: int, db: Session = Depends(get_db)):
    ...
    # Cascade: remove the subject's timetable slots and class instances so no
    # orphan rows (invisible in the UI but still counted in overall stats) remain.
    db.query(models.ClassInstance).filter(
        models.ClassInstance.subject_id == subject_id
